...
export interface SendResult {
  ...
  // "queued": true iff the remote host will never auto-push the row (floored, or the
  // recipient is not push-eligible there), false iff it is push-eligible there (the
  // remote heartbeat pushes it once due). Absent when the send was local or the remote
  // broker predates the field, so a reader must treat absence as "unknown", not
  // "push-eligible" (#39).
  poll_only?: boolean;
}
...
export interface ForwardMessageResponse {
  ...
  // true when floored (floor_remote_forwards on, push_after NULL) or when the recipient
  // is not push-eligible on this host at all (a delivery_kind='none' session, a paneless
  // row, or no tmux backend). Lets the originating broker tell its sender whether a
  // "queued" remote message waits for check_messages or still gets pushed by this host's
  // heartbeat. Absent from a broker predating the field, which the sender-side reader
  // treats as unknown rather than push-eligible (#39).
  poll_only?: boolean;
}
